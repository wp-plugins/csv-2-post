...
class CSV2POST {
    ...
    */
    ...
    /**
    * loads the plugin, we can exit early in this method if we do not need
    * the plugin to load.
    * 
    * @author Ryan Bayne
    * @package CSV 2 POST
    ...
    */
    public static function run() {
        /**
         * Fires when CSV 2 POST is loaded.
         *
         * @since 8.1.3
         */
        do_action( 'csv2post_run' );
    ...
    */
    ...
    /**
    * PDF Link Shortcode for use in templates only
    * 
    * This shortcode is to be used in a template only. 
    * Column replacement token will be used to populate the PDF document name.
    * Displays a link to a PDF file or a default image/text indicating no PDF document exists. 
    * We pass the PDF files name or part of it to the short-code.
    * We also pass the path to directory of PDF documents.
    *
    * add_shortcode( 'csv2postpdflink', 'pdflink_templateshortcode' );
    *                      
    * @param mixed $atts
    * 
    * @author Ryan Bayne
    ...
    * @version 1.0.2 
    */
    public function pdflink_templateshortcode( $atts ) {
        ...
        $path = content_url() .'/' . $a['dir'] .'/'. $a['file'] . '.pdf';

        //if(!file_exists( $path) ){  does not work with .pdf
        if(!@fopen( $path, 'rb' ) ){
            return __( 'No PDF available.' );
        }
        
        return '<img style="float: left; margin-right: 8px;" src="'. content_url() .'/images/adobe-pdf-icon-small.gif" alt="' . __( 'Open this PDF document', 'csv2post' ) . '" /><a href="' . $path . '" title="'. __( 'Open PDF Document in a new window', 'csv2post' ) . '" target="_blank">'. __( 'Open PDF Document in a new window', 'csv2post' ) . '</a>';
    }
    
    /**
    * Local Image Shortcode for use in templates only
    * 
    * This shortcode is to be used in a template only. 
    * Column replacement token will be used to populate the image name.
    * Displays the image a default image/text indicating no image exists. 
    * We pass the image files name or part of it to the short-code.
    * We also pass the path to directory of images.
    *
    * add_shortcode( 'csv2postlocalimage', 'localimage_templateshortcode' );
    *                      
    * @param mixed $atts
    * 
    * @author Ryan Bayne
    ...
    * @version 1.0.0 
    */
    public function localimage_templateshortcode( $atts ) {
        $a = shortcode_atts( array(
            'file' => false,
            'dir' => false,
            'attachid' => false,
        ...
        $path = content_url() .'/'. $a['dir'] .'/'. $a['file'] . '.jpg';

        //if(!file_exists( $path) ){
        if(!@fopen( $path, 'rb' ) ){
            return __( 'No image available.' );
        }
    ...
    */
    public function get_category_data_used( $project_id, $total = 1, $postid_as_key = false, $post_id = false ){
        ...
        $result = $WPDB->selectorderby( $from, $condition, $column, $select, $total, ARRAY_A );
    ...
    public function tables_exist( $tables_array ){
        if( $tables_array && is_array( $tables_array ) ){         
            ...
            foreach( $tables_array as $key => $table_name){
                $table_exists = C2P_DB::does_table_exist( $table_name);  
                if(!$table_exists){          
                    return false;
                }
            }        
...
class CSV2POST_InsertPost{
    ...
    */
    ...
    public function format() {
       ...
       if( isset( $project_array['postformat'] ) ){
            if( isset( $project_array['postformat']['title_table'] ) && isset( $project_array['postformat']['title_column'] ) ){
                if( isset( $record_array[ $project_array['postformat']['title_column'] ] ) && is_string( $record_array[ $project_array['postformat']['title_column'] ] ) ){
                    wp_set_post_terms( $this->my_post['ID'], 'post-format-'.$record_array[ $project_array['postformat']['title_column'] ], 'post_format' ); 
                }   
            }elseif( isset( $project_array['postformat']['default'] ) ){
                wp_set_post_terms( $this->my_post['ID'], 'post-format-'.$project_array['postformat']['default'], 'post_format' );    
            }
        }   
