...
if(isset($csv2post_textspin_array['settings']['status']) && $csv2post_textspin_array['settings']['status'] == true){
    ...
    csv2post_n_incontent('Curly braces method is not to be used in manually created posts or pages, let us know
    if you would like CSV 2 POST to do that. Please use curly braces method in your content templates and the spin
    will be done during post or category creation. <br /><br />
    Example: My name is {Ryan|Brian|Paul} and I live in {Scotland|America|Mars}','info','Small','Curly Braces Method');
...
++$panel_number;// increase panel counter so this panel has unique ID
$panel_array = csv2post_WP_SETTINGS_panel_array($pageid,$panel_number,$csv2post_tab_number);
$panel_array['panel_name'] = 'textspinningswitch';// slug to act as a name and part of the panel ID 
$panel_array['panel_title'] = __('Text Spinning Switch *global panel*');// user seen panel header text 
$panel_array['panel_id'] = $panel_array['panel_name'].$panel_number;// creates a unique id, may change from version to version but within a version it should be unique
$panel_array['panel_intro'] = __('Activation displays more options and uses more functions');
$panel_array['panel_help'] = __('Activation applies a function to all post creation. This extended
processing searches for one time spin tokens with in-line values i.e. textspininline#value example one#value
example two#value example three#textspininline. Activation also displays other accordion panels.');
$panel_array['help_button'] = csv2post_helpbutton_text(false,true);
// Form Settings - create the array that is passed to jQuery form functions
$jsform_set_override = array();
$jsform_set = csv2post_jqueryform_commonarrayvalues($pageid,$panel_array['tabnumber'],$panel_array['panel_number'],$panel_array['panel_name'],$panel_array['panel_title'],$jsform_set_override);     
$jsform_set['dialogbox_title'] = 'Activate Text Spin Features';
$jsform_set['noticebox_content'] = 'Text spinning requires each posts content to go through more processing, please activate if you intend to use text spinning only. Do you wish to continue?';
### TODO:LOWPRIORITY,improve the list of values so when many characters is used each line is clear?>
<?php csv2post_panel_header( $panel_array );?>
    ...
    <?php
    if(isset($csv2post_textspin_array['settings']['status']) && $csv2post_textspin_array['settings']['status'] == true){
        csv2post_n_incontent('Text spinning features are active. If you are not using them at all
        it is recommended you submit this form and disable them as it will reduce processing during 
        post creation.','info','Small','Text Spinning Is Active'); 
        
        $button_text = 'Disable Text Spinning';
                   
    }else{
        csv2post_n_incontent('Text spinning features are not active. To use any text spinning please
        submit this form. It is recommended that you do not activate text spinning unless you will
        use it.','info','Small','Text Spinning Not Active');
        
        $button_text = 'Activate Text Spinning';        
    }?>  
    ...
    <?php csv2post_formend_standard($button_text,$jsform_set['form_id']);?>
