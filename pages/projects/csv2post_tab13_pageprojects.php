...
<?php
if(isset($csv2post_project_array)){
    ++$panel_number;// increase panel counter so this panel has unique ID
    $panel_array = csv2post_WP_SETTINGS_panel_array($pageid,$panel_number,$csv2post_tab_number);
    $panel_array['panel_name'] = 'projecthistory';// slug to act as a name and part of the panel ID 
    $panel_array['panel_title'] = __('Project History');// user seen panel header text 
    $panel_array['panel_id'] = $panel_array['panel_name'].$panel_number;// creates a unique id, may change from version to version but within a version it should be unique
    ?>
    <?php csv2post_panel_header( $panel_array );?>
...
csv2post_log_display_bytype('project',100,$csv2post_logcats_array['categories']['project']['columns']);
?> 
    <?php csv2post_panel_footer();
}?>
