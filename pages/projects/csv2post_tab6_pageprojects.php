...
<?php   
### TODO:MEDIUMPRIORITY, for premium edition, use suggestions in the text field for known themes
...
if($csv2post_is_free){
    ...
    $panel_array['panel_title'] = __('Create Custom Field');// user seen panel header text 
    ...
    ++$panel_number;// increase panel counter so this panel has unique ID
    $panel_array = csv2post_WP_SETTINGS_panel_array($pageid,$panel_number,$csv2post_tab_number);
    $panel_array['panel_name'] = 'createpaidcustomfield';// slug to act as a name and part of the panel ID 
    $panel_array['panel_title'] = __('Setup Custom Field Rule');// user seen panel header text 
    $panel_array['panel_id'] = $panel_array['panel_name'].$panel_number;// creates a unique id, may change from version to version but within a version it should be unique
    $panel_array['panel_help'] = __('Setup as many custom fields as required by your theme and other plugins in use.');
    // Form Settings - create the array that is passed to jQuery form functions
    $jsform_set_override = array();
    $jsform_set = csv2post_jqueryform_commonarrayvalues($pageid,$panel_array['tabnumber'],$panel_array['panel_number'],$panel_array['panel_name'],$panel_array['panel_title'],$jsform_set_override);               
    $jsform_set['dialogbox_title'] = 'Save Custom Field Rule';
    ...
    ++$panel_number;// increase panel counter so this panel has unique ID
    $panel_array = csv2post_WP_SETTINGS_panel_array($pageid,$panel_number,$csv2post_tab_number);
    $panel_array['panel_name'] = 'createtemplatecustomfield';// slug to act as a name and part of the panel ID 
    $panel_array['panel_title'] = __('Create Template Custom Field');// user seen panel header text 
    $panel_array['panel_id'] = $panel_array['panel_name'].$panel_number;// creates a unique id, may change from version to version but within a version it should be unique
    ...
    <?php csv2post_panel_header( $panel_array );?>
        ...
        Enter Custom Field Key: <input type="text" name="csv2post_key2" />
        
        <br /><br />

        <script>
        $(function() {
            $( "#csv2post_advancedcustomfields_updatemeta2" ).buttonset();
        });
        </script>

        <div id="csv2post_advancedcustomfields_updatemeta2">
            Allow Updating: <input type="radio" id="csv2post_metaupdating_switch_on2" name="csv2post_updatesettings_metaupdating_switch_inputname2" value="on" />
            <label for="csv2post_metaupdating_switch_on2">Yes</label>
            <input type="radio" id="csv2post_metaupdating_switch_off2" name="csv2post_updatesettings_metaupdating_switch_inputname2" value="off" />
            <label for="csv2post_metaupdating_switch_off2">No</label>            
        </div>
        ...
        <div id="poststuff">
            ...
            <?php wp_editor('','csv2postwysiwygcf1',array('textarea_name' => 'csv2post_wysiwyg_customfield'));?>
        </div>        
