...
if($side == 'public'){  
    ...
    */
    function csv2post_ADDACTION_admin_init_registered_scripts() {
        ...
        if($version == 'original'){
            wp_register_script('jquery-multi-select',WTG_C2P_URL.'script/multi-select-basic/jquery.multi-select.js',array('jquery'));
        }elseif($version == 'latest'){ 
        ...
        // multi-select (lists, not the same as multiselect menus)
        if($version == 'original'){
            wp_register_script('jquery-cookie',WTG_C2P_URL.'script/jquery.cookie.js',array('jquery'));
        }elseif($version == 'latest'){ 
