...
function csv2post_get_file_headers_formatted($csv_filename,$fileid,$separator = ',',$quote = '"',$fields = 0){
    ...
    while ( ( $readone = File_CSV::read( WTG_C2P_CONTENTFOLDER_DIR . '/' . $csv_filename,$csv_file_conf ) ) ){                
        ...
        for ( $i = 0; $i < $csv_file_conf['fields']; $i++ ){
            ...
            $header_array[$i]['sql'] = csv2post_cleansqlcolumnname($readone[$i]);// none adapted/original sql version of headers, could have duplicates with multi-file jobs
            $header_array[$i]['sql_adapted'] = csv2post_cleansqlcolumnname($readone[$i]) . $fileid;// add files id to avoid duplicate header names              
        }           
