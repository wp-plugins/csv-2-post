...
function wtgcsv_notice($message,$type = 'success',$size = 'Extra',$title = false, $helpurl = 'http://www.wordpresscsvimporter.co.uk/support', $output_type = 'echo',$persistent = false,$clickable = false){
    if(is_admin()){
        ...
        if($output_type == 'return'){
            ...
            // if clickable we use a simplier function for building html content
            if($clickable){
                wtgcsv_notice_display_step($helpurl,$title,$message);
            }else{
...
*/
function wtgcsv_notice_display_step($helpurl,$title,$message){
    $output = '<div class="stepLargeTest"><a href="'.$helpurl.'">    
